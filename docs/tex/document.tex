	...
	\section{Rejstracja}
		\subsection{Opis przypadku użycia}
			...
			\begin{table}[h]
				...
				\begin{tabular}{|>{\columncolor[gray]{0.97}}p{4cm}| p{12cm} |} \hline
					\textbf{Aktorzy} & Użytkownik \\ \hline
					\textbf{Zakres}  & Sklep internetowy  \\ \hline
					\textbf{Poziom}  & Sytemowowy \\ \hline
					\textbf{Udziałowcy i cele} & Użytkownik chce stworzyć konto. Sklep chce zebrać dane spełniające zadane warunki. \\ \hline
					\textbf{\begin{tabular}[l]{@{}l@{}}Zdarzenie \\ wyzwalające\end{tabular}} & Użytkownik przechodzi na podstronę ‘Sign in/Register’ \\ \hline
					\textbf{Warunki wstępne} & Użytkownik nie jest zalogowany \\ \hline
					\textbf{\begin{tabular}[l]{@{}l@{}}Warunki końcowe \\ dla sukcesu\end{tabular}} & Konto użytkownika zostaje utworzone i zapisane w bazie danych. Użytkownik zostaje powiadomiony o sukcesie.  	\\ \hline
	...
	\section{Reklamacja}
		...
		\subsection{Opis przypadku użycia}
			...
			\begin{enumerate}
				...
				\item Użytkownik wprowadza dane w polu ‘Letter of complaint’
...
\chapter{Słownik danych}
	...
	%-------------------------------------------------------
		\subsubsection{A}
			\textbf{apartmentNumber} $=$ 1-4 cyfry
		\subsubsection{B}
			\textbf{buildingNumber} $=$ 1-4 cyfry 
		\subsubsection{C}
			\textbf{country} $=$ 1-20 liter  \\
			\textbf{city} $=$ 1-20 liter
		\subsubsection{E}
			\textbf{email} $=$ <minimum jeden znak> @ <minimum jeden znak> 
		\subsubsection{I}
			\textbf{firstName}  $=$ minimum jeden znak  
		\subsubsection{L}
			\textbf{lastName}  $=$ minimum jeden znak \\
			\textbf{letterOfComplaint} $=$ minimum 10 znaków
		\subsubsection{P}
			\textbf{password} $=$ minimum 8 znaków w tym wielka litera, mała litera i cyfra 
		\subsubsection{S}
			\textbf{street} $=$ 1-20 liter
		\subsubsection{Z}
			\textbf{ZIPcode} $=$ <cyfra><cyfra>-<cyfra><cyfra><cyfra>
